...
const dividBy3 = pokemon.filter(p => p.id % 3 === 0);
//console.log(dividBy3);

// an array of pokemon objects that are "fire" type
const fire = pokemon.filter( p => p.types[0] === "fire");
console.log(fire);
...
// an array wiht just the names of the pokemon
const justName = pokemon.map(p => p.name);
console.log(justName);

// an array with just the names of pokemon with an ide greater than 99
...
const countNormal = pokemon.filter(p => p.types.includes("normal")).map(p => p.name);
